...
(function (e) {
	...
	var w = e.document, s = w.documentElement, i = [], k = [], q = [], o = {}, h = 30, f = w.getElementsByTagName("head")[0] || s, g = w.getElementsByTagName("base")[0], b = f.getElementsByTagName("link"), d = [], a = function () {
		...
		for (; B < y; B++) {
			...
			if (!!z && x && !o[z]) {
				if (A.styleSheet && A.styleSheet.rawCssText) {
					...
					if ((!/^([a-zA-Z:]*\/\/)/.test(z) && !g) || z.replace(RegExp.$1, "").split("http://www.pxcreate.com/")[0] === e.location.host) {
		...
		var G = I.match(/@media[^\{]+\{([^\{\}]*\{[^\}\{]*\})+/gi), J = G && G.length || 0, x = x.substring(0, x.lastIndexOf("http://www.pxcreate.com/")), y = function (K) {
